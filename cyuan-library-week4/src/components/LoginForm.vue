<template>
    <div class="container mt-5">
        ...
        <!-- Card display for submitted data -->
...
const validateReason = (blur) => {
    ...
    const wordCount = reason.split(/\s+/).filter(word => word).length;
    const minWords = 5; // Adjust this number as needed for your minimum word count requirement

    if (wordCount < minWords) {
        if (blur) errors.value.reason = `Please provide a detailed reason with at least ${minWords} words.`;
    } else if (!wordPattern.test(reason.replace(/\s+/g, ' '))) {
        if (blur) errors.value.reason = "Reason must only contain English words and spaces.";
    } else {
